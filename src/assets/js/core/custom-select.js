...
// Auto-initialize custom selects when they are added to the DOM (for React compatibility)
        ...
        if (mutation.addedNodes.length > 0) {
            for (const node of mutation.addedNodes) {
                if (node.nodeType === 1) { // Element node
                    if (node.matches && node.matches('select[data-custom-select]')) {
                        shouldInit = true;
                    } else if (node.querySelector && node.querySelector('select[data-custom-select]')) {
                        shouldInit = true;
                    }
                }
            }
...
// For React, run global events init immediately since DOMContentLoaded may have already fired
initGlobalEvents();
