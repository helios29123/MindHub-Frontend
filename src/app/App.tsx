...
import {
  loadAndCleanUploadedSongs,
  saveUploadedSong,
  deleteUploadedSong,
  validateMp3File,
  PlayableSong,
} from "@/shared/utils/musicDb";
...
import CategoryFilterBar from "@/features/courses/components/CategoryFilterBar";
...
import { FreePreviewModal } from "@/features/courses/components/FreePreviewModal";
import ContactPage from "@/pages/ContactPage";
import AboutPage from "@/pages/AboutPage";
import { InstructorProfile } from "@/features/instructor/components/InstructorProfile";
import CourseQA from "@/features/classroom/components/CourseQA";
...
export default function App() {
  ...
  const [sortBy, setSortBy] = useState<
    "rating" | "bestseller" | "priceAsc" | "priceDesc" | "newest" | "oldest"
  >("newest");
  const [coursePurchaseFilter, setCoursePurchaseFilter] = useState<
    "all" | "unpurchased" | "purchased"
  >("all");
  ...
  const getRouteFromPath = (path: string) => {
    if (
      ...
      window.location.search.includes("route=vnpay-return")
    )
      return "vnpay-return";
    ...
    if (
      path.startsWith("/my-learning/favorites") ||
    ...
    if (path.match(/^\/admin\/([^/]+)\/dashboard/) || path.startsWith("/admin"))
      return "admin";
  ...
  const [profileSectionTab, setProfileSectionTab] = useState<
    "personal" | "security" | "preferences" | "activities"
  >("personal");
  const [coursesPage, setCoursesPage] = useState<number>(1);
  const [favSortBy, setFavSortBy] = useState<
    "favoritesDesc" | "ratingDesc" | "priceAsc"
  >("favoritesDesc");
  const [recentPurchasedFilter, setRecentPurchasedFilter] = useState<
    "1_week" | "2_weeks" | "3_weeks" | "1_month" | "all"
  >("all");
  ...
  const [viewedInstructorId, setViewedInstructorId] = useState<string | null>(
    null,
  );
  ...
  const [showMainSuggestions, setShowMainSuggestions] =
  ...
  const [isLoggedIn, setIsLoggedIn] = useState<boolean>(() => {
    try {
      ...
      console.error("Error parsing isLoggedIn:", e);
      return true;
    }
  });
  ...
  const getCoursePurchaseDate = (courseId: string): Date => {
    const matchedOrder = orders.find(
      (o) =>
        o.status === "success" &&
        o.courses.some((item) => item.id === courseId),
    );
    if (matchedOrder) {
  ...
  useEffect(() => {
    const handleOpenLegal = (e: Event) => {
      const customEvent = e as CustomEvent<{
        tab: "terms" | "privacy" | "refund" | "contact" | "faq";
      }>;
    ...
    return () =>
      window.removeEventListener("open-mindhub-legal", handleOpenLegal);
  }, []);
  ...
  useEffect(() => {
    ...
    setEnrolledCourseIds((prev) => {
      ...
      if (
        safePrev.length === merged.length &&
        safePrev.every((v) => merged.includes(v))
      ) {
        return safePrev;
      }
      return merged;
  ...
  const [directSelectCourseId, setDirectSelectCourseId] = useState<
    string | null
  >(null);
  const [initialCartTab, setInitialCartTab] = useState<"cart" | "wishlist">(
    "cart",
  );
  const [showLegal, setShowLegal] = useState<
    "terms" | "privacy" | "refund" | "contact" | "faq" | null
  >(null);

  const [showFloatingHelp, setShowFloatingHelp] = useState(false);
  const [showNotifications, setShowNotifications] = useState(false);
  const [isEditingProfile, setIsEditingProfile] = useState(false);
  const [profileActiveTab, setProfileActiveTab] = useState<"info" | "orders">(
    "info",
  );
  ...
  const [introAutoCloseCountdown, setIntroAutoCloseCountdown] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (introAutoCloseCountdown !== null) {
      if (introAutoCloseCountdown > 0) {
        timer = setTimeout(() => {
          setIntroAutoCloseCountdown((prev) =>
        ...
        const timer = setTimeout(() => {
          if (introVideoRef.current) {
            ...
            if (promise !== undefined) {
              promise.catch((err) => {
                console.warn(
                  "Autoplay was blocked by browser policy, user interaction required:",
                  err,
                );
              });
            }
  ...
  const PRESET_SONGS = [
    {
      id: "song-1",
      title: "☕ Lofi Sáng Tạo Quán Gỗ",
      url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3",
      isYoutube: false,
    },
    {
      id: "song-2",
      title: "🎷 Jazz Thu Yên Bình",
      url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3",
      isYoutube: false,
    },
    {
      id: "song-3",
      title: "🎹 Piano Tĩnh Tâm",
      url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3",
      isYoutube: false,
    },
    {
      id: "song-4",
      title: "🌿 Bản Giao Hưởng Cát",
      url: "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-4.mp3",
      isYoutube: false,
    },
    {
      id: "song-lofi-live",
      title: "🎧 Lofi Girl Live Trực Tuyến",
    ...
    try {
      ...
      if (!valResult.isValid) {
        setMusicNotification({
          text: valResult.error || "Tệp tin không hợp lệ.",
          type: "error",
        });
      ...
      setMusicNotification({
        text: `Tải lên "${file.name}" thành công! Nhạc sẽ lưu tối đa 1 tuần.`,
        type: "success",
      });
      setTimeout(() => setMusicNotification(null), 5000);
    } catch (err: any) {
      setMusicNotification({
        text: err.message || "Lỗi tải nhạc lên.",
        type: "error",
      });
      setTimeout(() => setMusicNotification(null), 5000);
    } finally {
  ...
  const handleDeleteSong = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      setMusicNotification({
        text: "Đã xóa bài hát thành công!",
        type: "success",
      });
      setTimeout(() => setMusicNotification(null), 3000);
    } catch (err: any) {
      setMusicNotification({
        text: err.message || "Gặp lỗi khi xóa bài hát.",
        type: "error",
      });
      setTimeout(() => setMusicNotification(null), 3000);
    }
  ...
  const [musicNotification, setMusicNotification] = useState<{
    text: string;
    type: "success" | "error";
  } | null>(null);
  ...
  useEffect(() => {
    ...
    if (!ytPlayerRef.current) {
          ...
          "youtube-bgm-placeholder",
          {
            ...
            events: {
              ...
              onStateChange: (event: any) => {
                const endedState =
                  (window as any).YT && (window as any).YT.PlayerState
      ...
      try {
        const currentVideoId = ytPlayerRef.current.getVideoData
  ...
  const handlePlayCustomYt = (e: React.FormEvent) => {
    ...
    if (ytId) {
      ...
      setYtUrlInput("");
      setMusicNotification({
        text: "Đã nhận dạng link YouTube! Bắt đầu phát BGM...",
        type: "success",
      });
      setTimeout(() => setMusicNotification(null), 4000);
    } else {
      setMusicNotification({
        text: "Link YouTube không tương thích! Hãy dán đúng địa chỉ hoặc id.",
        type: "error",
      });
      setTimeout(() => setMusicNotification(null), 4000);
    }
  ...
  const togglePlayMusic = (e?: React.MouseEvent) => {
    ...
    if (!currentSong.isYoutube && audioRef.current) {
      if (nextState) {
        audioRef.current
          .play()
          .catch((err) => console.log("BGM Playback error:", err));
      } else {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn(
        "[Audio Polyfill] Audio constructor is blocked or unsupported:",
        e,
      );
    }
  ...
  const [editedPhone, setEditedPhone] = useState(
    currentUser.phone || "098 765 4321",
  );
  ...
  const [demo1Notes, setDemo1Notes] = useState(
    "Sổ học thuật MindHub: Ghi lại kiến thức tâm đắc của bạn...",
  );
  ...
  const handleAccountClosureSubmit = async () => {
    ...
    if (!agreeToClosureTerms) {
      alert(
        "Vui lòng xác nhận bạn chịu trách nhiệm hoàn toàn đối với các rủi ro đã nêu.",
      );
      return;
    }
    ...
    try {
      ...
      setToastMessage(
        `Gửi yêu cầu ${closureType === "delete" ? "XÓA VĨNH VIỄN" : "TẠM KHÓA"} tài khoản thành công! Đơn của bạn đã được chuyển đến Ban Quản Trị Hệ Thống.`,
      );
    } catch (e: any) {
  ...
  const handleChangePassword = () => {
    alert(
      "Mã đặt lại mật khẩu an toàn đã được mã hóa gửi về hòm thư " +
        currentUser.email,
    );
  };
  ...
  const filteredCourses = useMemo(() => courses.filter((c) => {
    ...
    const matchesSubcat =
      selectedSubcategory === "All" || c.subcategory === selectedSubcategory;
    ...
    const isVisibleForCurrentRole =
      currentUser.role !== "student" || (c.status === "active" && !c.isHidden);
  ...
  const safeCoursesPage = Math.min(
    Math.max(1, coursesPage),
    totalCoursesPages || 1,
  );
  ...
  const getSearchSuggestions = (query: string) => {
    ...
    const suggestions: {
      type: "course" | "instructor" | "category" | "subcategory";
      value: string;
    }[] = [];
    ...
    categories.forEach((cat) => {
      if (
        ...
        !suggestions.some((s) => s.value === cat && s.type === "category")
      ) {
    ...
    const instructors = Array.from(
      new Set(courses.map((c) => c.instructorName)),
    ) as string[];
    instructors.forEach((inst) => {
      if (
        inst.toLowerCase().includes(q) &&
        !suggestions.some((s) => s.value === inst && s.type === "instructor")
      ) {
    ...
    const subcats = Array.from(
      new Set(courses.map((c) => c.subcategory)),
    ) as string[];
    subcats.forEach((sub) => {
      if (
        sub.toLowerCase().includes(q) &&
        !suggestions.some((s) => s.value === sub && s.type === "subcategory")
      ) {
    ...
    courses.forEach((c) => {
      if (
        ...
        !suggestions.some((s) => s.value === c.title && s.type === "course")
      ) {
...
setCoursesPage(1);
    ...
    setTimeout(() => {
      const el =
        document.getElementById("available-courses-section") ||
        document.getElementById("courses-explorer");
      if (el) {
    ...
    return (
      ...
      >
        <div className="relative">
          ...
          >
            <Heart
              className={`w-4 h-4 ${favorites.includes(c.id) ? "fill-deep-indigo text-deep-indigo" : "text-gray-400"}`}
            />
          </button>

          <div className="absolute bottom-2 left-2 flex gap-1 pointer-events-none">
            {c.isBestseller && (
              <span className="text-[8px] bg-yellow-500 text-brand-light px-2 py-0.5 rounded-md font-bold uppercase tracking-wider shadow-sm">
                Bán chạy
              </span>
            )}
            {c.isNew && (
              <span className="text-[8px] bg-blue-600 text-brand-light px-2 py-0.5 rounded-md font-bold uppercase tracking-wider shadow-sm">
                Mới nhất
              </span>
            )}
          </div>
          ...
          <div>
            ...
            <p
              ...
              onClick={(e) => {
                e.stopPropagation();
                navigateTo(
                  `instructor-profile?id=${c.instructorId || (c as any).instructor?.id || ""}`,
                );
              }}
            >
  ...
  return (
    <div className="min-h-screen bg-[#fbf9f6] flex flex-col text-main-darker selection:bg-brand-light-active selection:text-brand-dark relative overflow-hidden">
        ...
        <header className="bg-white border-b border-brand-light-active py-2 md:py-3 px-4 md:px-8 flex justify-between items-center sticky top-0 z-40 shadow-xs">
          ...
          <div className="flex items-center gap-2 md:gap-4 text-xs">
            ...
            <div className="flex items-center gap-1 sm:gap-1.5 pr-1 sm:pr-2 border-r border-brand-light-active select-none">
              ...
              >
                <Compass
                  className={`w-4 h-4 ${activeTab === "home" ? "text-white" : "text-brand-normal"}`}
                />
              ...
              >
                <Sparkles
                  className={`w-4 h-4 ${activeTab === "intro" ? "text-white" : "text-amber-500"}`}
                />
            ...
            {currentUser.role === "student" && (
              <>
                ...
                >
                  <Heart
                    className={`w-4.5 h-4.5 ${activeTab === "favorites" ? "fill-white text-white" : favorites.length > 0 ? "fill-deep-indigo text-deep-indigo" : "text-stone-500"}`}
                  />
                  {favorites.length > 0 && (
                ...
                >
                  <Users
                    className={`w-4 h-4 ${activeTab === "about" ? "text-white" : "text-emerald-500"}`}
                  />
                  <span className="hidden lg:inline">Giới thiệu</span>
                </button>
                ...
                >
                  <MessageSquare
                    className={`w-4 h-4 ${activeTab === "contact" ? "text-white" : "text-emerald-500"}`}
                  />
            ...
            >
              <HelpCircle
                className={`w-4 h-4 ${activeTab === "faq" ? "text-white" : ""}`}
              />
      ...
      {isAuthRoute ? (
        ...
        <main className={isFullScreenRoute ? "flex-1 relative z-10 w-full h-full" : "flex-1 p-4 md:p-12 max-w-7xl mx-auto w-full space-y-10 relative z-10"}>
              ...
              <div className="bg-[#1c1410] border border-stone-800 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl relative animate-scale-up aspect-video sm:aspect-[16/10] md:aspect-video flex flex-col">
                ...
                <div className="absolute top-3 right-3 sm:top-4 sm:right-4 z-50 flex items-center gap-2">
                  ...
                  {introAutoCloseCountdown !== null ? (
                    ...
                    <span className="bg-black/60 backdrop-blur-md border border-stone-700/50 text-[9px] text-stone-300 font-mono font-bold px-2.5 py-1 rounded-full">
                      Góc giới thiệu •{" "}
                      {Math.min(Math.floor(introVideoTime), 12)}s / 12s
                    </span>
                  )}
                ...
                >
                      ...
                      <div className="relative space-y-4 z-10 transition-transform duration-500 hover:scale-[1.02]">
                        <div className="flex justify-center">
                          <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-[#8b5e3c] to-[#eedecf]/80 shadow-2xl flex items-center justify-center relative animate-bounce [animation-duration:4s]">
                            ...
                            <svg
                              className="w-8 h-8 text-[#1c1410]"
                              fill="currentColor"
                              viewBox="0 0 24 24"
                            >
                              <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z" />
                            </svg>
                            <span className="absolute -top-1.5 -right-1.5 bg-amber-500 text-[8px] text-white font-extrabold px-1.5 py-0.5 rounded-full uppercase tracking-wider">
                              Demo
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="space-y-1 max-w-sm">
                          ...
                          <p className="text-[10.5px] text-stone-300 leading-relaxed max-w-xs mx-auto pt-0.5 font-sans">
                            Sửa đổi dòng{" "}
                            <code className="bg-stone-900/80 px-1 py-0.5 rounded text-[#eedecf] font-mono text-[9.5px]">
                              INTRO_VIDEO_URL
                            </code>{" "}
                            trong{" "}
                            <code className="bg-stone-900/80 px-1 py-0.5 rounded text-amber-400 font-mono text-[9.5px]">
                              src/App.tsx
                            </code>{" "}
                            bất kỳ lúc nào để liên kết trực tiếp video của bạn.
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-5 sm:p-7 md:p-8 space-y-2 pointer-events-none text-left">
                    ...
                    <div className="flex gap-1 mb-2">
                      {[0, 1, 2, 3].map((idx) => {
                        const isActive =
                          Math.min(Math.floor(introVideoTime / 3), 3) === idx;
                        return (
                          <div
                            key={idx}
                    ...
                    {Math.min(Math.floor(introVideoTime / 3), 3) === 0 && (
                      <div className="animate-fade-in-up space-y-1.5">
                        ...
                        <p className="text-xs sm:text-[13px] text-stone-200 leading-relaxed font-sans max-w-lg">
                          Không gian trực tuyến tinh túy kết hợp giữa tri học
                          truyền thống và công nghệ giáo dục hiện đại.
                        </p>
                      </div>
                    )}
                    ...
                    {Math.min(Math.floor(introVideoTime / 3), 3) === 1 && (
                      <div className="animate-fade-in-up space-y-1.5">
                        ...
                        <p className="text-xs sm:text-[13px] text-stone-200 leading-relaxed font-sans max-w-lg">
                          Học viên theo dõi cụ thể tiến độ với bài giảng trực
                          quan, làm bài thực hành chuẩn mực cao.
                        </p>
                      </div>
                    )}
                    ...
                    {Math.min(Math.floor(introVideoTime / 3), 3) === 2 && (
                      <div className="animate-fade-in-up space-y-1.5">
                        ...
                        <p className="text-xs sm:text-[13px] text-stone-200 leading-relaxed font-sans max-w-lg">
                          Sở hữu quầy pha trà, espresso thư thái và thư viện
                          nhạc nền đa dạng cho cảm hứng bay bổng.
                        </p>
                      </div>
                    )}
                    ...
                    {Math.min(Math.floor(introVideoTime / 3), 3) === 3 && (
                      <div className="animate-fade-in-up space-y-1.5">
                        ...
                        <p className="text-xs sm:text-[13px] text-stone-200 leading-relaxed font-sans max-w-lg">
                          Lựa chọn học phần, rèn luyện chăm chỉ và thăng hoa mỗi
                          ngày ngay trên hòn đảo học tập này!
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="h-1 bg-stone-800 w-full relative">
                  <div
                    ...
                    style={{
                      width: `${Math.min((introVideoTime / 12) * 100, 100)}%`,
                    }}
                  />
                </div>
              </div>
              ...
              {showPasswordPage ? (
                ...
                <div className="bg-white rounded-2xl max-w-sm w-full p-5 sm:p-6 space-y-4 text-left text-xs text-main-darker animate-fade-in border shadow-2xl relative max-h-[85vh] overflow-y-auto tactile-scrollbar">
                  <h3 className="font-display font-semibold text-[#8b5e3c] text-sm flex items-center gap-1.5 border-b pb-2">
                    <Lock className="w-4 h-4 text-[#8b5e3c]" /> Đặt Lại Mật Khẩu
                    Tài Khoản
                  </h3>
                ...
                <form
                  ...
                  className="bg-white rounded-2xl max-w-2xl w-full p-6 space-y-5 text-left text-xs text-main-darker animate-fade-in border shadow-2xl max-h-[90vh] overflow-y-auto tactile-scrollbar relative"
                >
                  <div className="flex items-center justify-between border-b pb-3.5">
                    <div>
                      <h3 className="font-display font-semibold text-main-normal text-sm md:text-base">
                        Hồ sơ cá nhân & Cấu hình học tập
                      </h3>
                      <p className="text-[10px] text-gray-550 mt-0.5 font-sans">
                        Cập nhật thông tin nhận diện học thuật và cấu hình các
                        đề xuất của bạn.
                      </p>
                    </div>
                    <button
                  ...
                  <div className="flex gap-4 items-center bg-stone-50 border border-stone-200/80 p-3.5 rounded-xl">
                    <img
                      src={editedAvatar}
                      alt="Avatar Preview"
                      className="w-12 h-12 rounded-full border border-brand-normal object-cover shadow-sm bg-white"
                    />
                    <div className="text-left flex-1 space-y-1">
                      <div className="flex flex-wrap items-center gap-1.5">
                        <span className="font-bold text-xs block text-stone-850 font-mono">
                          {currentUser.email}
                        </span>
                        <span className="bg-emerald-50 text-emerald-800 border border-emerald-200 px-1.5 py-0.5 rounded flex items-center gap-0.5 font-bold text-[8px] uppercase font-mono tracking-wider">
                          <Check className="w-2.5 h-2.5 text-emerald-600" /> Đã
                          xác minh
                        </span>
                      </div>
                      <p className="text-[10px] text-stone-500 flex flex-wrap items-center gap-1.5 mt-0.5">
                        <span className="bg-stone-200 text-stone-700 px-1.5 py-0.5 rounded uppercase font-mono font-black text-[8px] tracking-wider">
                          {currentUser.role}
                        </span>
                        <span>
                          Học lực tích lũy:{" "}
                          <b>{currentUser.streak} ngày streak</b> liên tiếp
                        </span>
                  ...
                  {profileActiveTab === "info" ? (
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-1">
                        ...
                        <div className="space-y-4">
                          <div className="bg-slate-50/50 p-3.5 rounded-xl border border-slate-100 space-y-3">
                            <h4 className="font-bold text-[#8b5e3c] uppercase tracking-widest text-[9px] border-b pb-1">
                              1. Định danh cá nhân
                            </h4>
                                ...
                                {AVAILABLE_AVATARS.map((url, i) => (
                                  <button
                                    ...
                                    className={`w-9 h-9 rounded-full overflow-hidden border-2 transition-all shrink-0 cursor-pointer ${
                                      editedAvatar === url
                                        ? "border-[#8b5e3c] ring-2 ring-[#8b5e3c]/40 scale-105"
                        ...
                        <div className="space-y-4">
                          <div className="bg-slate-50/50 p-3.5 rounded-xl border border-slate-100 space-y-3">
                            <h4 className="font-bold text-[#8b5e3c] uppercase tracking-widest text-[9px] border-b pb-1">
                              2. Trải nghiệm học học thuật
                            </h4>

                            <div>
                              <label className="block text-[11px] font-bold text-stone-700 mb-1">
                                Tiểu sử học tập ngắn:
                              </label>
                              <input
                                type="text"
                                value={editedBio}
                                ...
                                {PREDEFINED_TOPICS.map((topic) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      key={topic}
                                      onClick={() =>
                                        handleTogglePredefinedTopic(topic)
                                      }
                            ...
                            <div className="pt-2 border-t flex flex-col gap-1.5 font-sans">
                              ...
                              >
                                <Lock className="w-3 h-3 text-[#8b5e3c]" /> Đặt
                                lại mật khẩu tài khoản bảo mật
                              </button>
                              <button
                                type="button"
                              ...
                              >
                                ...
                                Đề xuất Đóng/Xóa vĩnh viễn tài khoản
                              </button>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="space-y-4 pt-1">
                      <div className="flex items-center justify-between">
                        <h4 className="font-bold text-[#8b5e3c] uppercase tracking-widest text-[9.5px]">
                      ...
                      {orders.length === 0 ? (
                        <div className="text-center py-10 border border-dashed rounded-xl bg-slate-50/50">
                          <p className="text-stone-550 italic">
                            Bạn chưa thực hiện bất kỳ giao dịch mua khóa học
                            nào.
                          </p>
                        </div>
                      ) : (
                        <div className="space-y-3 max-h-[50vh] overflow-y-auto pr-1">
                          {orders.map((or) => {
                            ...
                            return (
                              <div
                                key={or.id}
                                className="border border-stone-200 rounded-xl p-3.5 space-y-2.5 bg-white hover:border-stone-300 transition-all text-xs"
                              >
                                <div className="flex justify-between items-start flex-wrap gap-2 pb-2 border-b border-stone-100">
                                  <div className="space-y-0.5 text-left">
                                    <p className="font-bold font-mono text-stone-850">
                                      Mã đơn: #{or.id}
                                    </p>
                                    <p className="text-[10px] text-stone-400">
                                      Ngày đặt mua: {or.date}
                                    </p>
                                  </div>
                                ...
                                <div className="space-y-1 text-left">
                                  {or.courses.map((c) => (
                                    ...
                                    >
                                      <span className="font-medium text-stone-750 truncate max-w-[280px]">
                                        {c.title}
                                      </span>
                                      <span className="font-mono text-stone-550">
                                        {formatVND(c.price)}
                                      </span>
                                    </div>
                                  ))}
                                ...
                                <div className="flex justify-between font-bold text-xs pt-1.5 border-t border-stone-100 text-left">
                                  <span className="text-stone-700 uppercase tracking-wider text-[9.5px]">
                                    Tổng thanh toán:
                                  </span>
                                  <span className="text-[#852b21]">
                                    {formatVND(or.total)}
                                  </span>
                                </div>
                                        ...
                                        <button
                                          ...
                                          onClick={() => {
                                            ...
                                            const relatedCourse = courses.find(
                                              (c) => c.id === or.courses[0]?.id,
                                            );
                                            alert(
                                              `Tuyệt vời! Thanh toán cho đơn hàng ${or.id} đã được kích hoạt thành công. Bạn đã được tự động ghi danh vào lớp "${relatedCourse?.title || "Khóa học"}".`,
                                            );
                                          }}
                                    ...
                                    {or.status === "success" && (
                                      <button
                                        ...
                                        onClick={() => {
                                          ...
                                          if (relatedCourse) {
                                            setIsEditingProfile(false);
                                            setDirectSelectCourseId(
                                              relatedCourse.id,
                                            );
                                          } else {
                                            alert(
                                              "Khóa học không tìm thấy hoặc đã bị tháo rỡ.",
                                            );
                                          }
                                        }}
              ...
              <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 border border-stone-200 shadow-2xl relative max-h-[90vh] overflow-y-auto tactile-scrollbar">
                <div className="flex items-center justify-between border-b pb-3.5">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h3 className="font-display font-bold text-stone-900 text-sm md:text-base">
                        Đơn Yêu Cầu Khóa / Xóa Tài Khoản
                      </h3>
                      <p className="text-[10px] text-gray-500 leading-none">
                        Chuyển lên Ban Quản Trị Hệ Thống thẩm định
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <div className="space-y-4 text-xs text-stone-850">
                  <div className="bg-stone-50 border border-stone-200/80 p-3.5 rounded-xl space-y-1">
                    <span className="text-[10px] text-stone-400 font-mono block">
                      THÔNG TIN THÀNH VIÊN ĐỀ XUẤT:
                    </span>
                  ...
                  <div className="space-y-2">
                    <label className="block text-[11px] font-bold text-stone-700">
                      Hình thức xử lý đề xuất:
                    </label>
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      >
                        ...
                        <p className="text-[10px] text-stone-500 leading-tight mt-1">
                          Bảo lưu toàn bộ tiến trình học, điểm tích lũy & chứng
                          chỉ. Có thể mở khóa bất kỳ lúc nào.
                        </p>
                      </button>
                      ...
                      >
                        ...
                        <p className="text-[10px] text-stone-500 leading-tight mt-1">
                          Hủy bỏ vĩnh viễn mọi dữ liệu, tiến độ học, quyền đăng
                          nhập & khóa học đã mua.
                        </p>
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-[11px] font-bold text-stone-700">
                      Lý do khóa / xóa tài khoản (Bắt buộc):
                    </label>
                    <textarea
                      rows={4}
                      value={closureReason}
                  ...
                  >
                    <p className="font-bold flex items-center gap-1 text-xs">
                      ⚠️{" "}
                      {closureType === "delete"
                        ? "Cảnh báo Bảo mật Xóa vĩnh viễn:"
                        : "Lưu ý Tại khóa tài khoản:"}
                    </p>
                    <p className="mt-1">
                      {closureType === "delete"
                  ...
                  <div className="flex items-start gap-2.5 pt-1.5">
                    ...
                    />
                    <label
                      htmlFor="confirm-closure-terms"
                      className="text-[11px] text-stone-605 leading-snug select-none cursor-pointer font-medium"
                    >
                      Tôi cam kết đã thấu hiểu đầy đủ tác động bảo mật và chấp
                      nhận chịu trách nhiệm cho quyết định này.
                    </label>
                  </div>
            ...
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4 animate-fade-in text-left">
              <div className="bg-white rounded-2xl max-w-sm w-full p-6 space-y-4 border border-stone-200 shadow-2xl">
                ...
                <div className="text-center">
                  <h3 className="font-display font-bold text-stone-850 text-sm">
                    Xác nhận đăng xuất
                  </h3>
                  <p className="text-[11px] text-stone-500 mt-1.5 leading-relaxed">
                    Bộ lưu trữ sẽ đưa bạn về vai trò khách tham quan. Bạn có
                    muốn đăng xuất khỏi phiên làm việc này không?
                  </p>
                </div>
          ...
          {toastMessage && (
            <div className="fixed top-24 right-4 sm:right-6 md:right-8 bg-stone-900/95 border border-stone-850 text-white shadow-2xl p-3.5 px-4 rounded-2xl z-[10000] max-w-sm flex items-start gap-2.5 animate-fade-in text-xs font-semibold backdrop-blur-xs text-left">
              <span className="text-emerald-400 shrink-0 select-none font-bold">
                ✓
              </span>
          ...
          {viewedCourse ? (
            <div className="space-y-8 animate-fade-in text-left">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#e8ded3] pb-4">
                ...
                <div className="flex items-center gap-3">
                  <span className="font-bold text-xs tracking-tight text-stone-500 font-sans">
                    Chế độ hiển thị:
                  </span>
              ...
              <div
                className={`rounded-3xl overflow-hidden shadow-sm border transition-all duration-300 ${isDetailDarkMode ? "bg-[#1c1210] border-[#382622] text-[#eee5db]" : "bg-[#faf9f6] border-[#e8ded3] text-stone-800"}`}
              >
                {/* Cover Banner */}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6 md:p-8">
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    {/* What you'll learn */}
                    {viewedCourse.willLearn &&
                      viewedCourse.willLearn.length > 0 && (
                        <div
                          className={`p-5 rounded-2xl ${isDetailDarkMode ? "bg-white/5 border border-white/10" : "bg-white/95 border border-[#e8ded3] shadow-3xs"}`}
                        >
                          <h4 className="font-bold text-[#432c28] text-sm mb-3">
                            Bạn sẽ gặt hái được gì?
                          </h4>
                    ...
                    {/* Requirements */}
                    {viewedCourse.requirements &&
                      viewedCourse.requirements.length > 0 && (
                        <div className="space-y-2">
                          <h4 className="font-bold text-sm text-[#432c28]">
                            Yêu cầu học tập:
                          </h4>
                          <ul className="list-inside text-xs space-y-2 pl-2 text-justify">
                            {viewedCourse.requirements.map((item, idx) => (
                              <li
                                key={idx}
                                className={`flex items-start gap-2 ${isDetailDarkMode ? "text-stone-300" : "text-stone-600"}`}
                              >
                    ...
                    <div className="space-y-3">
                      <h4 className="font-bold text-sm font-sans text-[#432c28]">
                        Nội dung chương trình đào tạo (
                        {viewedCourse.chapters?.length || 0} chương học):
                      </h4>
                      {viewedCourse.chapters &&
                        viewedCourse.chapters.map((chapter) => (
                          <div
                            key={chapter.id}
                            className={`p-4 rounded-xl border ${isDetailDarkMode ? "bg-white/5 border-white/5 text-stone-300" : "bg-white border-[#e8ded3] text-stone-800"} text-xs text-left`}
                          >
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-[10px] text-stone-400 shrink-0 font-mono italic">
                                {chapter.lessons.length} bài •{" "}
                                {(chapter as any).duration || "45 phút"}
                              </span>
                            </div>
                            <ul className="mt-2.5 space-y-2 pl-3 border-l border-[#8b5e3c]/20">
                              {chapter.lessons.map((lesson) => {
                                ...
                                const isPreviewable =
                                  !isEnrolled &&
                                  ((lesson.type === "doc" &&
                                    viewedCourse.allowFreeDoc) ||
                                    (lesson.type === "video" &&
                                      viewedCourse.allowFreeVideo));
                                return (
                                  <li
                                    key={lesson.id}
                                    className="text-[11px] text-stone-600 flex items-center justify-between hover:text-stone-900 py-1 border-b border-stone-100 last:border-none"
                                  >
                    ...
                    <div
                      className={`p-5 rounded-2xl flex flex-col md:flex-row gap-4 items-start ${isDetailDarkMode ? "bg-white/5 border border-white/10" : "bg-[#faf6f2] border border-[#e8ded3]"}`}
                    >
                      <img
                        src={viewedCourse.instructorAvatar}
                        alt="Instructor"
                        className="w-14 h-14 rounded-full border border-[#8b5e3c] shrink-0 object-cover"
                      />
                      <div>
                        <span
                          className={`font-bold text-xs block ${isDetailDarkMode ? "text-brand-light-active" : "text-[#432c28]"}`}
                        >
                          {viewedCourse.instructorName}
                        </span>
                        <span className="text-[10px] text-[#8b5e3c] block font-medium mt-0.5">
                          {viewedCourse.instructorTitle}
                        </span>
                        <p
                          className={`text-[11px] italic mt-2 leading-relaxed text-left ${isDetailDarkMode ? "text-stone-305" : "text-stone-600"}`}
                        >
                          {viewedCourse.instructorBio}
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="space-y-3 text-left">
                        ...
                        <div className="space-y-2">
                          {viewedCourse.faqs.map((faq, idx) => (
                            <details
                              key={idx}
                              className={`p-3 rounded-xl border text-xs cursor-pointer ${isDetailDarkMode ? "bg-white/5 border-white/5 text-stone-300" : "bg-white border-[#e8ded3] text-stone-800"}`}
                            >
                              ...
                              <p className="mt-2 text-stone-600 leading-relaxed pl-4 border-l-2 border-[#8b5e3c] text-left whitespace-pre-wrap">
                                {faq.a || faq.answer}
                              </p>
                            </details>
                          ))}
                    ...
                    <div className="space-y-4 text-left">
                      <h4 className="font-bold text-sm flex items-center gap-1.5 text-[#432c28]">
                        <Star className="w-4 h-4 text-[#8b5e3c] fill-[#8b5e3c] shrink-0" />{" "}
                        Đánh giá học viên ({viewedCourse.reviews?.length || 0})
                      </h4>
                      {viewedCourse.reviews &&
                      viewedCourse.reviews.length > 0 ? (
                        viewedCourse.reviews.map((rev) => (
                          <div
                            key={rev.id}
                            className={`p-4 rounded-xl border text-xs space-y-2 ${isDetailDarkMode ? "bg-[#221715] border-white/5 text-stone-300" : "bg-white border-[#e8ded3] text-stone-800 shadow-3xs"}`}
                          >
                            <div className="flex justify-between items-center">
                              <span className="font-bold text-[#432c28]">
                                {rev.userName ||
                                  (rev as any).user ||
                                  "Học viên"}
                              </span>
                        ...
                        <p className="text-xs text-stone-400 italic">
                          Chưa có lượt đánh giá nào cho khóa học này.
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div
                      className={`p-6 rounded-2xl border ${isDetailDarkMode ? "bg-[#281b18] border-[#3d2a25]" : "bg-[#faf6f2] border-[#e8ded3]"} space-y-4 text-left shadow-xs sticky top-32`}
                    >
                        ...
                        {enrolledCourseIds.includes(String(viewedCourse.id)) ? (
                          ...
                          <div className="space-y-1">
                            ...
                            <span className="text-xs text-stone-400 line-through">
                              Giá gốc: {formatVND(viewedCourse.price)}
                            </span>
                          </div>
                        ) : (
                          <span className="text-xl md:text-2xl font-black text-[#432c28] block">
                            {formatVND(viewedCourse.price)}
                          </span>
                        )}
                      </div>
            ...
            currentUser.id === routeUserId ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
                ...
                <p className="text-stone-500 mb-6 text-center max-w-md">
                  Bạn không có quyền truy cập vào Khu vực Giảng viên này hoặc
                  URL không hợp lệ.
                </p>
                <button
                  onClick={() => navigateTo("/")}
                  className="px-6 py-2.5 bg-brand-normal text-white font-bold rounded-xl shadow-md hover:bg-brand-dark transition-all cursor-pointer"
                >
                  Về Trang Chủ
                </button>
              </div>
            ...
            currentUser.role === "admin" ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
                ...
                <p className="text-stone-500 mb-6 text-center max-w-md">
                  Bạn không có quyền quản trị viên để vào trang này.
                </p>
                <button
                  onClick={() => navigateTo("/")}
                  className="px-6 py-2.5 bg-brand-normal text-white font-bold rounded-xl shadow-md hover:bg-brand-dark transition-all cursor-pointer"
...
setCoursesPage(1);
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10">
                        <div className="lg:col-span-7 space-y-4 md:space-y-5">
                          ...
                          <p className="text-xs md:text-sm text-stone-605 leading-relaxed font-sans max-w-xl">
                            Nền tảng học tập trực tuyến chắt lọc tinh hoa từ
                            phương pháp giáo dục truyền thống mộc mạc và trải
                            nghiệm công nghệ tinh tế nhất. Hôm nay bạn đang thắp
                            sáng ngọn lửa học tập cùng MindHub!
                          </p>
                          ...
                          <div className="pt-3 border-t border-brand-light-active/40 flex gap-4 items-center text-[11px] text-stone-500">
                            <div>
                              <span className="font-serif italic text-base font-bold text-[#432c28] block">
                                15k+
                              </span>
                              <span className="text-[8px] uppercase tracking-widest font-bold text-stone-400">
                                HỌC VIÊN
                              </span>
                            </div>
                            ...
                            <div className="w-px h-6 bg-brand-light-active/50"></div>
                            <div>
                              <span className="font-serif italic text-base font-bold text-[#8b5e3c] block font-semibold">
                                4.9/5
                              </span>
                              <span className="text-[8px] uppercase tracking-widest font-bold text-stone-400">
                                ĐÁNH GIÁ
                    ...
                    <div className="bg-white border border-brand-light-hover rounded-3xl p-6 md:p-8 space-y-6 shadow-2xs">
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-[#faf5ec] pb-4 gap-2 text-left">
                        <div>
                          <h3 className="text-xl md:text-2xl font-extrabold text-[#432c28] flex items-center gap-2">
                            <BookOpen className="w-5 h-5 text-brand-normal hover:scale-110 transition-transform" />{" "}
                            Góc Trực Quan MindHub: Trực Tiếp Trải Nghiệm Tinh
                            Hoa
                          </h3>
                          <p className="text-stone-500 text-[11.5px] mt-1">
                            Cùng ngắm nhìn và <b>tương tác thử trực tiếp</b> với
                            ba trải nghiệm học tập độc quyền tại lớp học số mộc
                            mạc của chúng tôi.
                          </p>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="group rounded-2xl overflow-hidden border border-brand-light-active hover:border-brand-normal/40 transition-all duration-300 bg-[#faf7f2]/30 flex flex-col justify-between p-2 min-h-[530px] md:min-h-[550px]">
                          <div>
                            ...
                            <div className="p-4 text-left space-y-3.5">
                              <h4 className="font-extrabold text-[#432c28] text-sm md:text-base">
                                01. Trải nghiệm học tĩnh lặng
                              </h4>
                              <p className="text-stone-500 text-[11px] leading-relaxed">
                                Từng bài học được tinh gọn mộc mạc tựa cuốn sổ
                                tay da bên tách Latte thơm ngát. Thử bật nhạc âm
                                cảnh gỗ để đạt tập trung cao độ:
                              </p>
                              ...
                              <div className="pt-1">
                                ...
                                >
                                  {demo1AudioActive
                                    ? "✕ Tắt Tiếng Mưa Rào mộc"
                                    : "🎧 Phát Tiếng Mưa Gỗ Cafe (Thư Giãn)"}
                                </button>
                              </div>
                              ...
                              {demo1AudioActive && (
                                <div className="flex gap-1 items-end h-4 justify-center py-1 bg-amber-50 rounded-lg">
                                  ...
                                  <span className="text-[9px] font-mono text-[#8b5e3c] font-bold">
                                    Đang phát brown-noise mịn...
                                  </span>
                                </div>
                              )}
                              ...
                              <div className="space-y-1">
                                <span className="text-[9px] uppercase tracking-wider font-bold text-stone-400 block">
                                  Sổ tay nháp ý tưởng trực tuyến:
                                </span>
                                <textarea
                                  value={demo1Notes}
                                  onChange={(e) =>
                                    setDemo1Notes(e.target.value)
                                  }
                                  rows={2}
                        ...
                        <div className="group rounded-2xl overflow-hidden border border-brand-light-active hover:border-brand-normal/40 transition-all duration-300 bg-[#faf7f2]/30 flex flex-col justify-between p-2 min-h-[530px] md:min-h-[550px]">
                          <div>
                            ...
                            <div className="p-4 text-left space-y-3.5">
                              <h4 className="font-extrabold text-[#432c28] text-sm md:text-base">
                                02. Học liệu chắt lọc tinh hoa
                              </h4>
                              <p className="text-stone-500 text-[11px] leading-relaxed">
                                Tri thức tại đây không dàn trải xô bồ ngoài thị
                                trường. Hãy bấm "Xem thử" giáo trình cô đọng 3
                                chương cốt lõi ngay:
                              </p>
                              ...
                              <div className="space-y-1.5 pt-1">
                                {[
                                  {
                                    num: 1,
                                    title: "Bản chất vòng đời State React",
                                    desc: "Sự khác nhau cơ bản giữa render queue, batching gom cụm cập nhật mới nhất và useRef giữ tham chiếu bảo mật.",
                                  },
                                  {
                                    num: 2,
                                    title: "Clean Architecture mộc mạc",
                                    desc: "Phục dựng kết nối decoupled cô lập side-effects, phân tầng logic API trong server đem lại điểm 10 tin cậy sạch sẽ.",
                                  },
                                  {
                                    num: 3,
                                    title: "Nghệ thuật đàm thoại prompt AI",
                                    desc: "Thiết lập mô hình tư duy hội thoại bám sát văn hóa địa phương Việt để tối ưu năng suất làm việc gấp 10 lần.",
                                  },
                                ].map((ch) => (
                                  <div
                                    key={ch.num}
                                    className="bg-white border border-stone-150 rounded-xl p-2.5 text-left transition-all hover:border-[#8b5e3c]/50"
                                  >
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                    ...
                                    >
                                      ...
                                      <span className="text-[9px] text-[#8b5e3c] shrink-0 font-extrabold font-mono">
                                        {demo2Chapter === ch.num
                                          ? "▲ Đóng"
                                          : "▼ Xem"}
                                      </span>
                        ...
                        <div className="group rounded-2xl overflow-hidden border border-brand-light-active hover:border-brand-normal/40 transition-all duration-300 bg-[#faf7f2]/30 flex flex-col justify-between p-2 min-h-[530px] md:min-h-[550px]">
                          <div>
                            ...
                            <div className="p-4 text-left space-y-3.5">
                              <h4 className="font-extrabold text-[#432c28] text-sm md:text-base">
                                03. Cố vấn học tập AI chu đáo
                              </h4>
                              <p className="text-stone-500 text-[11px] leading-relaxed">
                                Nhận diện lỗ hổng kiến thức chuẩn xác. Mời bạn
                                trả lời thử câu hỏi mấu chốt được đưa ra bởi AI
                                Mentor:
                              </p>
                                ...
                                {demo3QuizStep === 0 ? (
                                  <div className="space-y-1.5 text-left">
                                    <p className="font-bold text-[10.5px] text-[#432c28] leading-tight">
                                      Q: Có nên dùng thuộc tính "index" trong
                                      mảng làm giá trị cho thuộc tính "key" khi
                                      render trong React?
                                    </p>
                                    <div className="flex flex-col gap-1 pt-1.5">
                                      ...
                                      <button
                                        ...
                                        className="w-full bg-stone-50 hover:bg-[#8b5e3c]/5 border border-stone-100 p-2 rounded-lg text-left text-[10px] text-stone-750 font-bold cursor-pointer transition-all"
                                      >
                                        B. Hạn chế, vì có thể gây xáo trộn sai
                                        lệch state
                                      </button>
                                    </div>
                                  </div>
                                  ...
                                  <div className="space-y-1.5 animate-fade-in text-left">
                                    {demo3QuizStep === 2 ? (
                                      <>
                                        <span className="text-[9px] bg-emerald-50 text-emerald-800 px-2 py-0.5 rounded font-bold uppercase inline-block font-mono font-bold">
                                          Đúng rồi! ✓
                                        </span>
                                        <p className="text-[10px] text-stone-600 leading-normal font-sans pt-1">
                                          React định vị thẻ dựa trên "key" để
                                          tối thiểu re-renders. Khi bạn
                                          Sort/Filter mảng, index bị xáo sẽ làm
                                          React gắn nhầm state nội bộ của thẻ cũ
                                          sang thẻ mới, tạo bug giao diện nguy
                                          hại khó lường!
                                        </p>
                                      </>
                                    ) : (
                                      <>
                                        <span className="text-[9px] bg-amber-50 text-amber-800 px-2 py-0.5 rounded font-bold uppercase inline-block font-mono font-bold">
                                          Chưa chuẩn xác! ✕
                                        </span>
                                        <p className="text-[10px] text-stone-600 leading-normal font-sans pt-1">
                                          Dùng index làm key là phản mẫu
                                          (anti-pattern) khi list mảng bị sắp
                                          xếp lại, xóa chèn phần tử. Hãy ưu tiên
                                          dùng id duy nhất (uuid, db id) để
                                          React đối sánh DOM ảo một cách tuyệt
                                          đối chính xác!
                                        </p>
                                      </>
                                    )}
                      ...
                      <div className="bg-[#faf7f2] border-l-3 border-[#8b5e3c] p-4 rounded-r-xl text-center">
                        <p className="italic text-xs md:text-sm text-[#432c28] leading-relaxed">
                          "Tri thức đích thực không nằm ở lượng video ta tích
                          góp, mà nằm ở trải nghiệm tinh thần an nhiên tựa những
                          giờ phút suy ngẫm bên bàn gỗ trà chiều."
                        </p>
                      </div>
                    </div>
                  </ScrollReveal>
                  ...
                  <ScrollReveal delayMs={200}>
                    <div className="bg-[#fcfcfc] border border-stone-150 p-5 rounded-3xl flex flex-col md:flex-row justify-between items-center gap-4 text-left shadow-2xs">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="font-extrabold text-[#432c28] text-sm">
                            Có câu hỏi về MindHub?
                          </h4>
                          <p className="text-stone-500 text-xs">
                            Chính sách cam kết hoàn trả 100% học liệu số trong 7
                    ...
                    <div className="space-y-5 border-t border-brand-light-active pt-6 mt-6">
                      ...
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-3 pb-3 border-b border-brand-light-active text-left w-full">
                        <div className="space-y-1">
                          <h3 className="text-lg md:text-xl font-display font-extrabold flex items-center gap-1.5 text-main-normal leading-tight">
                            <Users className="w-5 h-5 text-brand-normal" /> Đội
                            ngũ Giảng viên Chuyên môn MindHub
                          </h3>
                          <p className="text-stone-500 text-[11px] md:text-xs">
                            Tìm sự đồng hành lý tưởng từ các cựu kỹ sư Google,
                            chuyên gia thiết kế Figma và diễn giả Harvard uyên
                            bác.
                          </p>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          <div className="flex items-center gap-1.5">
                            <span className="text-[10px] text-stone-500 font-bold uppercase tracking-wide select-none">
                              Lọc lĩnh vực:
                            </span>
                            <select
                          ...
                          >
                            <Star
                              className={`w-3.5 h-3.5 ${showOnlyFeatured ? "fill-amber-400 text-amber-500" : "text-stone-400"}`}
                            />
                            <span>Xem Giảng Viên Nổi Bật</span>
                            {showOnlyFeatured && (
                              <span className="ml-1 w-2 h-2 bg-brand-normal rounded-full inline-block animate-ping" />
                            )}
                          </button>
                        </div>
                      </div>
                      ...
                      {(() => {
                        const filtered = INSTRUCTORS_DATA.filter((inst) => {
                          const matchesCat =
                            instructorCategory === "All" ||
                            inst.expertise === instructorCategory;
                          const matchesFeat =
                            !showOnlyFeatured || inst.isFeatured;
                          ...
                          return (
                            <div className="text-center py-10 text-stone-400 bg-white border border-dashed rounded-2xl w-full">
                              <p className="text-xs font-semibold">
                                Không tìm thấy giảng viên tiêu chuẩn nào phù hợp
                                với bộ lọc đã chọn.
                              </p>
                            </div>
                          );
                              ...
                              >
                                ...
                                <div className="space-y-3.5">
                                  ...
                                  <div className="flex gap-4">
                                    ...
                                    />
                                    <div className="min-w-0">
                                      <h4 className="font-display font-bold text-[#432c28] text-sm truncate">
                                        {inst.name}
                                      </h4>
                                      <p className="text-[10px] text-stone-400 leading-tight truncate mt-0.5">
                                        {inst.title}
                                      </p>
                                ...
                                <div className="flex items-center justify-between text-[11px] pt-3 border-t border-stone-50">
                                  <div className="flex items-center gap-1">
                                    ...
                                    <span className="text-stone-400">
                                      ({inst.studentsCount.toLocaleString()} học
                                      viên)
                                    </span>
                                  </div>
                  ...
                  <ScrollReveal delayMs={200}>
                    <div className="bg-[#fcfcfc] border border-stone-150 rounded-3xl p-5 md:p-6 space-y-4 shadow-3xs text-left">
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-stone-100 pb-3">
                        <div className="space-y-0.5">
                          <h3 className="text-base md:text-lg font-extrabold flex items-center gap-2 text-[#432c28]">
                            <BookOpen className="w-5 h-5 text-[#8b5e3c]" /> Khóa
                            học đã mua gần đây
                          </h3>
                          <p className="text-[11px] text-stone-500">
                            Hiển thị danh sách khóa học bạn sở hữu dựa trên mốc
                          ...
                          return (
                            <div className="text-center py-8 text-stone-400 bg-white border border-dashed border-stone-200 rounded-2xl">
                              <p className="text-xs font-medium">
                                Bạn chưa sở hữu khóa học nào. Hãy khám phá các
                                khóa học chất lượng cao của chúng tôi dưới đây!
                              </p>
                            </div>
                          );
                        }
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                            {recentEnrolledCourses.map((c) => {
                              ...
                              return (
                                <div
                                  key={c.id}
                                  className="bg-white border border-stone-150 rounded-2xl p-4 flex flex-col justify-between hover:shadow-md transition-all text-left space-y-3 relative"
                                >
                                  <div className="flex gap-3">
                                    ...
                                    <div className="min-w-0 flex-1">
                                      <div className="flex flex-wrap gap-1 items-center">
                                        <span className="text-[8px] bg-stone-100 text-stone-600 px-1.5 py-0.5 rounded font-mono font-bold uppercase">
                                          {c.subcategory}
                                        </span>
                                      ...
                                      <h4
                                        className="font-bold text-xs text-main-normal truncate mt-1.5"
                                        title={c.title}
                                      >
                                        {c.title}
                                      </h4>
                          ...
                          {(() => {
                                ...
                                : [
                                    {
                                      ...
                                      subtitle:
                                        "Nền tảng học tập trực tuyến chắt lọc tinh hoa từ phương pháp giáo dục truyền thống và trải nghiệm công nghệ tinh tế. Hôm nay bạn đang thắp sáng ngọn lửa học tập!",
                            ...
                            return (
                              <div
                                style={{
                                  backgroundColor:
                                    currentBanner.backgroundColor || "#fcf8f2",
                                  color: currentBanner.textColor || "#432c28",
                                }}
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-center relative z-10">
                                  ...
                                  <div className="lg:col-span-7 space-y-4 lg:space-y-5">
                                    ...
                                    <h1 className="text-2xl md:text-3xl lg:text-4xl font-extrabold tracking-tight leading-[1.1]">
                                      ...
                                      !
                                      <span
                                        className="block font-serif italic font-normal text-2xl md:text-3xl lg:text-[36px] mt-2 leading-[1.15]"
                                        style={{
                                          color: currentBanner.accentColor,
                                        }}
                                      >
                                    ...
                                    <div className="pt-3 border-t border-brand-light-active/40 flex gap-4 items-center text-[11px] text-stone-500">
                                      ...
                                      <div className="w-px h-6 bg-brand-light-active/50"></div>
                                      <div>
                                        <span className="font-serif italic text-base font-bold text-[#432c28] block font-semibold text-[#8b5e3c]">
                                          4.9/5
                                        </span>
                                        <span className="text-[8px] uppercase tracking-widest font-bold text-stone-400">
                                          ĐÁNH GIÁ
                    ...
                    <div className="mt-8 space-y-8">
                      ...
                      <ScrollReveal delayMs={200}>
                        <div className="bg-white border border-stone-200 rounded-3xl p-5 md:p-6 shadow-3xs">
                          <h3 className="text-base md:text-lg font-extrabold flex items-center gap-2 text-[#432c28] border-b border-stone-100 pb-3 mb-4">
                            <Award className="w-5 h-5 text-amber-500" /> Khóa
                            học đã hoàn thành
                          </h3>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                              {completedCoursesList
                                ...
                                .map((enrollment: any) => (
                                  <div
                                    key={enrollment.id}
                                    className="border border-stone-200 rounded-2xl p-3 flex gap-3 hover:border-[#8b5e3c] transition-colors bg-stone-50/50"
                                  >
                                    ...
                                    <div className="flex flex-col justify-between py-1">
                                      <div>
                                        <h4 className="font-bold text-stone-800 text-xs line-clamp-2">
                      ...
                      <ScrollReveal delayMs={250}>
                        <div className="bg-white border border-stone-200 rounded-3xl p-5 md:p-6 shadow-3xs">
                          <div className="flex justify-between items-center border-b border-stone-100 pb-3 mb-4">
                            <h3 className="text-base md:text-lg font-extrabold flex items-center gap-2 text-[#432c28]">
                              <Clock className="w-5 h-5 text-indigo-500" /> Lịch
                              sử học tập
                            </h3>
                            {learningHistory.length > 5 && (
                              <button
                                onClick={() => navigateTo("profile")}
                                className="text-xs text-[#8b5e3c] font-bold hover:underline cursor-pointer"
                              >
                                Xem tất cả »
                              </button>
                            )}
              ...
              {activeTab === "my-courses" && (
                <div className="space-y-6 text-sm text-left animate-fade-in px-4 md:px-8 py-6 max-w-7xl mx-auto">
                  <div className="border-b border-brand-light-active pb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                      ...
                      <h2 className="text-xl md:text-2xl font-display font-black text-main-darker flex items-center gap-2">
                        <BookOpen className="w-6 h-6 text-brand-normal" /> Danh
                        Sách Khóa Học Đã Tham Gia
                      </h2>
                      <p className="text-xs text-stone-500 mt-1">
                        Quản lý và theo dõi toàn bộ tiến trình các khóa học bạn
                        đang tham gia trên hệ thống.
                      </p>
                    </div>
                      ...
                      <div className="flex flex-wrap items-center gap-3 shrink-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              setMyCoursesStatus(e.target.value);
                              setMyCoursesPage(1);
                            }}
                            className="border border-stone-250 rounded-xl px-3 py-1.5 bg-white focus:outline-none focus:border-deep-indigo text-xs font-bold text-stone-750 shadow-3xs cursor-pointer hover:border-stone-300 transition-all"
                          >
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <select
                            value={myCoursesSort}
                            onChange={(e) => {
                              setMyCoursesSort(e.target.value);
                              setMyCoursesPage(1);
                            }}
                            className="border border-stone-250 rounded-xl px-3 py-1.5 bg-white focus:outline-none focus:border-deep-indigo text-xs font-bold text-stone-750 shadow-3xs cursor-pointer hover:border-stone-300 transition-all"
                          >
                            ...
                            : Array.from(
                                new Set(
                                  courses
                                    .filter(
                                      (c) =>
                                        c && c.category === myCoursesParentCat,
                                    )
                                    .map((c) => c?.subcategory)
                                    .filter(Boolean),
                                ),
                  ...
                  {(() => {
                    const allEnrolled = Array.isArray(courses)
                      ? courses.filter(
                          (c) =>
                            c &&
                            Array.isArray(enrolledCourseIds) &&
                      ...
                      return (
                        <div className="text-center py-16 text-stone-400 bg-white border border-dashed border-stone-300 rounded-3xl shadow-3xs p-8 space-y-4">
                          ...
                          <p className="text-xs font-medium text-stone-500 max-w-md mx-auto">
                            Hãy khám phá các khóa học tinh hoa trên hệ thống và
                            đăng ký để bắt đầu hành trình học tập ngay hôm nay!
                          </p>
                          <div className="pt-2">
                            <button
                              onClick={() => navigateTo("home")}
                    ...
                    const filtered = allEnrolled
                      .filter((c) => {
                        // Search filter
                        if (myCoursesSearch.trim()) {
                          const q = myCoursesSearch.toLowerCase();
                          const matchTitle = c.title.toLowerCase().includes(q);
                          const matchInst = c.instructorName
                            .toLowerCase()
                            .includes(q);
                          const matchCat =
                            c.category.toLowerCase().includes(q) ||
                            c.subcategory.toLowerCase().includes(q);
                          if (!matchTitle && !matchInst && !matchCat)
                            return false;
                        }
                        // Parent category filter
                        if (
                          myCoursesParentCat !== "All" &&
                          c.category !== myCoursesParentCat
                        )
                          return false;
                        // Child category filter
                        if (
                          myCoursesChildCat !== "All" &&
                          c.subcategory !== myCoursesChildCat
                        )
                          return false;
                        // Status filter
                        const backendProgress = enrolledCoursesData.find((ed: any) => String(ed.course?.id || ed.id) === String(c.id))?.progress_percent;
                        ...
                        if (myCoursesStatus === "not_started" && compRate > 0)
                          return false;
                        if (
                          myCoursesStatus === "in_progress" &&
                          (compRate === 0 || compRate === 100)
                        )
                          return false;
                        if (myCoursesStatus === "completed" && compRate < 100)
                          return false;
                        return true;
                      })
                      .sort((a, b) => {
                        if (
                          myCoursesSort === "recent" ||
                          myCoursesSort === "newest"
                        ) {
                          const timeA =
                            (a as any).lastAccessedAt ||
                            (a as any).updatedAt ||
                    ...
                    if (filtered.length === 0) {
                      return (
                        <div className="text-center py-16 text-stone-400 bg-white border border-dashed border-stone-300 rounded-3xl shadow-3xs p-8 space-y-3">
                          ...
                          <p className="text-xs font-medium text-stone-500 max-w-md mx-auto">
                            Không có khóa học nào khớp với bộ lọc hoặc từ khóa
                            tìm kiếm hiện tại. Vui lòng thử lại với các tiêu chí
                            khác.
                          </p>
                          <button
                            onClick={() => {
                              ...
                              >
                                ...
                                <div className="p-5 space-y-4 flex-1 flex flex-col justify-between">
                                  <div>
                                    ...
                                    <h4
                                      className="text-[15px] font-bold text-main-darker leading-snug line-clamp-2 hover:text-brand-normal cursor-pointer mt-1"
                                      onClick={() => setViewedCourse(c)}
                                    >
                                      {c.title}
                                    </h4>
                  ...
                  <div className="border-b border-brand-light-active pb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                      ...
                      <h1 className="text-2xl md:text-3xl font-display font-extrabold text-[#432c28] flex items-center gap-2">
                        <Heart className="w-7 h-7 fill-deep-indigo text-deep-indigo" />{" "}
                        Danh sách Khóa học Yêu thích
                      </h1>
                      <p className="text-stone-500 text-xs mt-1">
                        Tổng hợp các khóa học bạn đang quan tâm và mong muốn sở
                        hữu trong tương lai.
                      </p>
                    </div>
                    ...
                    if (favorites.length === 0) {
                      return (
                        <div className="py-16 text-center bg-white border border-stone-200 border-dashed rounded-3xl p-8 max-w-2xl mx-auto space-y-4 shadow-3xs">
                          <div className="w-16 h-16 bg-stone-100 rounded-full flex items-center justify-center mx-auto text-stone-300 text-2xl font-bold">
                            ♡
                          </div>
                          <h3 className="font-display font-extrabold text-lg text-[#432c28]">
                            Danh sách yêu thích trống
                          </h3>
                          <p className="text-stone-500 text-xs leading-normal">
                            Bạn chưa thêm khóa học nào vào danh sách yêu thích.
                            Hãy khám phá hàng trăm khóa học chất lượng cao trên
                            MindHub và bấm biểu tượng trái tim để lưu lại nhé!
                          </p>
                          <button
                            onClick={() => navigateTo("home")}
                      ...
                      return (
                        <div className="text-center py-12 bg-white border border-stone-200 border-dashed rounded-3xl p-8 max-w-2xl mx-auto space-y-2">
                          <p className="font-bold text-stone-700 text-sm">
                            Không tìm thấy khóa học yêu thích phù hợp
                          </p>
                          <p className="text-stone-500 text-xs">
                            Vui lòng thử điều chỉnh lại từ khóa tìm kiếm hoặc bộ
                            lọc danh mục.
                          </p>
                        </div>
                      );
                      ...
                      <div className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {filtered.map((c) => {
                            ...
                            return (
                              <div
                                key={c.id}
                                className="border border-brand-light-active rounded-3xl p-5 bg-white flex flex-col justify-between hover:shadow-xl transition-all text-left space-y-4 relative group"
                              >
                                <button
                                  onClick={() => {
                                ...
                                <div className="space-y-3">
                                  ...
                                  <div className="space-y-1.5">
                                    ...
                                    <h4 className="font-extrabold text-base text-[#432c28] group-hover:text-brand-normal transition-colors line-clamp-2 leading-snug">
                                      {c.title}
                                    </h4>
                                  </div>
                                </div>
                                ...
                                <div className="flex items-center justify-between pt-3 border-t border-stone-100">
                                  <div className="space-y-0.5">
                                    {c.salePrice ? (
                                      <div className="flex flex-col">
                                        <span className="text-base font-extrabold text-[#432c28]">
                                          {formatVND(c.salePrice)}
                                        </span>
                        ...
                        <div className="flex justify-center pt-4">
                          <p className="text-stone-400 text-xs font-medium">
                            Hiển thị toàn bộ {filtered.length} khóa học yêu
                            thích
                          </p>
                        </div>
                      </div>
                ...
                <div className="space-y-6 text-sm text-left animate-fade-in px-4 md:px-8 py-6 max-w-5xl mx-auto">
                  <div className="border-b border-brand-light-active pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      ...
                      <h1 className="text-2xl md:text-3xl font-display font-extrabold text-[#432c28] flex items-center gap-2">
                        <Bell className="w-7 h-7 text-brand-normal" /> Quản lý
                        Thông báo cá nhân
                      </h1>
                      <p className="text-stone-500 text-xs mt-1">
                        Theo dõi thông báo lịch học, cập nhật hệ thống và các sự
                        kiện diễn ra tại MindHub.
                      </p>
                    </div>
                    ...
                    if (filtered.length === 0) {
                      return (
                        <div className="py-16 text-center bg-white border border-stone-200 border-dashed rounded-3xl p-8 max-w-2xl mx-auto space-y-3 shadow-3xs">
                          ...
                          <h3 className="font-display font-bold text-base text-[#432c28]">
                            Không có thông báo nào
                          </h3>
                          <p className="text-stone-500 text-xs">
                            {notifFilter === "unread"
                              ? "Bạn đã đọc toàn bộ thông báo. Không có thông báo mới nào!"
                              : "Chưa có thông báo nào trong danh sách này."}
                          </p>
                        </div>
                      );
                    }
                          ...
                          <div
                            ...
                            className={`p-5 rounded-2xl border transition-all flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 ${
                              n.read
                                ...
                                : "bg-brand-light/40 border-brand-light-active text-[#432c28] shadow-3xs font-medium"
                            }`}
                          >
                            <div className="space-y-1.5 flex-1 pr-4">
                              <div className="flex items-center gap-2">
                                {!n.read && (
                                  <span className="w-2 h-2 bg-brand-normal rounded-full shrink-0 animate-pulse"></span>
                                )}
                                {n.targetCourseId &&
                                  n.targetCourseId !== "all" && (
                                    <span className="bg-amber-100 text-amber-900 px-2 py-0.5 rounded text-[9px] font-mono font-extrabold uppercase">
                                      LỚP THAM GIA
                                    </span>
                                  )}
                                <h4 className="font-bold text-sm text-stone-850">
                                  {n.title}
                                </h4>
                              </div>
              ...
              {activeTab === "profile" &&
                (currentUser.id === routeUserId ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
                    ...
                    <p className="text-stone-500 mb-6 text-center max-w-md">
                      Bạn chỉ có thể xem hoặc chỉnh sửa hồ sơ cá nhân của chính
                      mình.
                    </p>
                    <button
                      onClick={() => navigateTo("/")}
                ...
                <CartAndCheckout
                  ...
                  onEnrollSuccess={(newEnrolledIds, order) => {
                    if (newEnrolledIds.length > 0) {
                      ...
                      setFavorites((prev) =>
                        prev.filter((id) => !newEnrolledIds.includes(id)),
                      );
                    }
              ...
              {(activeTab === "legal" || activeTab === "faq" || showLegal) && (
                <FooterLegal
                  initialTab={
                    showLegal || (activeTab === "faq" ? "faq" : "terms")
                  }
                  onClose={() => {
        ...
        {showFloatingHelp && (
          <div className="absolute bottom-16 right-0 w-64 bg-white rounded-2xl border border-[#e8ded3] shadow-2xl p-4.5 space-y-3.5 text-left text-stone-800 animate-scale-up">
            <div className="border-b border-[#e8ded3]/60 pb-2">
              <span className="font-bold text-xs text-[#8b5e3c] block">
                💬 Trợ Giúp & Hỗ Trợ
              </span>
              <span className="text-[10px] text-stone-400 mt-0.5 block leading-tight">
                MindHub Việt Nam - Trực tuyến 24/7 giải đáp thắc mắc
              </span>
            </div>
        ...
        >
          {showFloatingHelp ? (
            ...
            <>
              ...
              <span className="absolute top-0 right-0 w-3 h-3 bg-red-500 rounded-full border border-white flex items-center justify-center font-bold text-[7px] text-white animate-pulse">
                !
              </span>
            </>
          )}
