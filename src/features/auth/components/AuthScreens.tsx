...
export default function AuthScreens({
  ...
  const [localRegisteredUsers, setLocalRegisteredUsers] = useState<UserType[]>(
    () => {
      ...
      seeded.forEach((u: any) => {
        u.password = "password123";
      });
      localStorage.setItem(
        "mindhub_registered_users_db",
  ...
  const [loginHistory, setLoginHistory] = useState<UserType[]>(() => {
    ...
    const defaultHistory = [
      {
        ...SYSTEM_ROLE_USERS.student,
        isEmailVerified: true,
        password: "password123",
      },
      {
        ...SYSTEM_ROLE_USERS.instructor,
        isEmailVerified: true,
        password: "password123",
      },
      {
        ...SYSTEM_ROLE_USERS.admin,
        isEmailVerified: true,
        password: "password123",
      },
    ] as any[];
    localStorage.setItem(
      "mindhub_logged_in_history",
  ...
  const mapAuthError = (err: any) => {
    if (err instanceof ApiError) {
      if (err.status === 401) {
        ...
        return Array.isArray(firstErr)
          ? firstErr[0]
          : err.message || "Dữ liệu đăng nhập không hợp lệ.";
  ...
  const handleSeedClick = (seed: (typeof DB_SEED_ACCOUNTS)[0]) => {
    ...
    setErrorMsg("");
    setSuccessMsg(
      `Đã điền tài khoản mẫu: ${seed.email}. Đang kết nối xác thực...`,
    );
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    authApi
      .register({
        ...
        role: registerRole,
        expertise:
          registerRole === "instructor" ? instructorSpecialty : undefined,
        bio: registerRole === "instructor" ? instructorBio : undefined,
        experience_years:
          registerRole === "instructor" ? instructorExperience : undefined,
      })
      .then((res: any) => {
  ...
  const handleForgot = (e: React.FormEvent) => {
    ...
    authApi
      .requestPasswordReset(emailTrimmed)
      .then(() => {
        setSuccessMsg(
    ...
    <div className="min-h-[85vh] bg-stone-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="bg-deep-indigo p-5 text-brand-light flex items-center justify-between border-b-4 border-emerald-500 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h2 className="text-base sm:text-lg font-suisseintl font-bold tracking-tight text-[#f5ece3] leading-tight">
                MindHub Academic Portal
              </h2>
              <p className="text-[10px] text-brand-light/80 font-suisseintlmono uppercase tracking-wider">
                Học thuật và Quản trị tri thức
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 tactile-scrollbar space-y-4">
            ...
            <div>
              <form onSubmit={handleLogin} className="space-y-4 text-left">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <div className="flex justify-between mb-1">
                      <label className="text-xs font-semibold text-stone-605">
                        Mật khẩu
                      </label>
                      <button
                        type="button"
                ...
                <div className="relative my-3 flex items-center justify-center">
                  ...
                  <span className="relative bg-white px-2.5 text-[10px] text-stone-400 font-mono">
                    HOẶC DÙNG GOOGLE
                  </span>
                </div>
                ...
                >
                  {googleLoading ? (
                    ...
                    <svg className="w-4 h-4 shrink-0" viewBox="0 0 24 24">
                      <path
                        fill="#EA4335"
                        d="M12 5.04c1.62 0 3.08.56 4.22 1.65l3.15-3.15C17.45 1.74 14.89 1 12 1 7.35 1 3.39 3.65 1.45 7.5l3.6 2.79C6.01 7.23 8.79 5.04 12 5.04z"
                      />
                      <path
                        fill="#4285F4"
                        d="M23.49 12.27c0-.81-.07-1.59-.2-2.34H12v4.44h6.46c-.28 1.48-1.12 2.73-2.38 3.58l3.63 2.81c2.13-1.97 3.78-4.87 3.78-8.49z"
                      />
                      <path
                        fill="#FBBC05"
                        d="M5.05 10.29c-.24-.73-.38-1.5-.38-2.29s.14-1.56.38-2.29L1.45 2.92C.53 4.75 0 6.81 0 9s.53 4.25 1.45 6.08l3.6-2.79z"
                      />
                      <path
                        fill="#34A853"
                        d="M12 23c3.24 0 5.97-1.09 7.96-2.96l-3.63-2.81c-1.1.74-2.51 1.18-4.33 1.18-3.21 0-5.99-2.19-6.95-5.25l-3.6 2.79C3.39 20.35 7.35 23 12 23z"
                      />
                    </svg>
                  )}
                  {googleLoading
            ...
            <form onSubmit={handleRegister} className="space-y-4 text-left">
              <div>
                ...
                <p className="text-[11px] text-stone-500 mt-1">
                  Cơ hội trải nghiệm học tập đỉnh cao tại hòn đảo tri thức của
                  chúng tôi.
                </p>
              </div>
              ...
              {registerRole === "instructor" && (
                <div className="bg-amber-50/50 p-3 rounded-xl border border-amber-100 space-y-1">
                  <span className="text-[10px] uppercase tracking-wider font-bold text-[#8b5e3c]">
                    Chế độ Đăng ký Đối tác Giảng dạy
                  </span>
                  <p className="text-[11px] text-stone-500">
                    Form này dành riêng cho các Thầy cô muốn phát triển và xuất
                    bản học liệu trực tuyến trên MindHub.
                  </p>
                </div>
              )}
              ...
              <div className="flex items-start gap-2 bg-stone-50 p-3 rounded-xl border">
                ...
                <label
                  htmlFor="agree"
                  className="text-[11px] text-stone-600 leading-normal select-none"
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      window.dispatchEvent(
                        new CustomEvent("open-mindhub-legal", {
                          detail: { tab: "terms" },
                        }),
                      )
                    }
              ...
              >
                <UserPlus className="w-4 h-4" /> Đăng Ký Tài Khoản & Gửi OTP Xác
                Thực
              </button>
              ...
              <div className="text-center pt-2 border-t border-stone-105 mt-2">
                <p className="text-xs text-stone-505">
                  ...
                  <button
                    ...
                    className="text-[#8b5e3c] font-black hover:underline"
                  >
                    Quay về Đăng nhập
                  </button>
                </p>
          ...
          {mode === "verify-email" &&
            (() => {
              const currentRegisteredUser = localRegisteredUsers.find(
                (u) => u.email.toLowerCase() === email.trim().toLowerCase(),
              );
              const activeOtp =
                currentRegisteredUser?.verificationOtp || "123456";
              return (
                <form
                  onSubmit={handleVerify}
                  className="space-y-4 text-center max-w-sm mx-auto py-4"
                >
                  ...
                  <h3 className="text-base font-bold text-stone-850">
                    Xác thực OTP Email của bạn
                  </h3>
                  <p className="text-xs text-stone-500">
                    Hệ thống bảo mật kiểm soát thư rác đã gửi mã bảo vệ 6 chữ số
                    đến <b>{email || "bạn"}</b> để phê duyệt tài khóa.
                  </p>
                    ...
                    <div className="bg-amber-50/80 border border-amber-200 rounded-xl p-3.5 text-[11px] text-amber-800 text-left mt-4 space-y-1 shadow-3xs">
                      ...
                      <p className="leading-normal">
                        ...
                        <span className="block text-center my-1.5">
                          <b className="font-mono text-sm bg-amber-100 border border-amber-200 px-2.5 py-1 rounded text-amber-950 font-black tracking-widest select-all">
                            {activeOtp}
                          </b>
                        </span>
          ...
          {mode === "forgot-password" && (
            <form
              ...
              className="space-y-4 max-w-sm mx-auto text-left py-4"
            >
              <h3 className="text-base font-bold text-stone-850">
                Nhận Mã Khôi Phục Mật Khẩu
              </h3>
              <p className="text-xs text-stone-500 leading-normal">
                Không sao cả! Hãy cung cấp hòm thư thành viên của bạn. Hệ thống
                sẽ cấp mã khôi phục cho bạn ngay.
              </p>
          ...
          {mode === "reset-password" &&
            (() => {
              ...
              const activeOtp =
                (currentRegisteredUser as any)?.resetOtp || "123456";
              return (
                <form
                ...
                >
                  ...
                  <p className="text-xs text-stone-500 leading-normal">
                    Vui lòng nhập Mã xác thực đã gửi cho tài khoản{" "}
                    <b>{email}</b> và điền mật khẩu mới của bạn bên dưới.
                  </p>

                  <div>
                    <label className="block text-xs font-semibold text-stone-605 mb-1">
                      Mã OTP Khôi Phục (6 chữ số)
                    </label>
                    <div className="relative">
                      <input
                        type="text"
        ...
        <div className="bg-stone-50 border-t border-stone-200/80 p-4 shrink-0 flex justify-between items-center text-[10px] text-stone-400">
          <span className="flex items-center gap-1 font-mono">
            <Shield className="w-3 h-3 text-emerald-600" /> 2-FACTOR SECURED
          </span>
          <button
            type="button"
            onClick={onClose}
            className="text-[#8b5e3c] hover:text-black font-bold"
          >
            Đóng cửa sổ [Esc]
          </button>
        </div>
