...
export const authApi = {
  ...
  async login(payload: any): Promise<{ user: User; token: string }> {
    devLog("Auth", "Login credentials authentication", {
      email: payload.email,
    });
  ...
  async logoutAll(): Promise<{ success: boolean }> {
    ...
    const res = await apiFetch<{ success: boolean }>("/auth/logout-all", {
      method: "POST",
    });
    setAuthToken(null);
    return res;
  ...
  async revokeSession(sessionId: string): Promise<{ success: boolean }> {
    ...
    return apiFetch<{ success: boolean }>(`/auth/sessions/${sessionId}`, {
      method: "DELETE",
    });
  },
  ...
  async resendVerificationEmail(
    ...
    try {
      const res = await fetch(
        "http://localhost:3000/api/auth/email/send-verification",
        {
          method: "POST",
  ...
  async sendOtpForContactChange(
    ...
    return new Promise((resolve) =>
      setTimeout(
        () => resolve({ success: true, message: "OTP đã được gửi" }),
        1000,
      ),
    );
  },

  async verifyOtpContactChange(
    field: "email" | "phone",
    newValue: string,
    otp: string,
  ...
  async sendChangePasswordOtp(payload: {
    ...
    success: boolean;
    message: string;
    data?: { expires_in: number; resend_after: number; masked_email: string };
  }> {
  ...
  async verifyPhoneOtp(
    ...
    try {
      const res = await fetch(
        "http://localhost:3000/api/auth/phone/verify-otp",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async sendInstructorPayoutAccountOtp(
  ...
  async requestInstructorEarlyWithdrawalOtp(payload: {
    amount: number;
