...
import { Course, Chapter, Lesson, QuizQuestion } from '@/shared/types';
import { 
...
export interface CourseBuilderWizardProps {
  currentUser: any;
  courses: Course[];
...
Hãy viết một hàm đệ quy để giải quyết bài toán lồng thư mục và tối ưu hóa thời gian chạy O(n).`;
  ...
  useEffect(() => {
    ...
    if (initialBuilderStep) {
      setBuilderStep(initialBuilderStep);
    }
  }, [initialEditCourseId, initialBuilderStep]);
...
<div className="space-y-6 animate-fade-in text-xs text-left bg-slate-50/50 p-5 rounded-2xl border border-stone-200">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 border-b pb-3.5">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  onClick={() => {
                    if (window.confirm('Quay lại và bỏ qua tất cả chưa lưu?')) onCloseWizard();
                  }}
